				...
				<section role="main" class="content-body">
					...
					<div class="row">
						<div class="content">
							...
							<form method="post" action="<?= base_url('admin/entrytransaksi'); ?>">
							<br>
							   <div class="input-group input-search">
									<label for="">Pilih Pelanggan</label><br/>
									<select style="width:200px;" name="pelanggan" id="" class="form-control">
									    <option>Pilih</option>
										<?php foreach($pesanan as $dataa) : ?>
										<option value="<?= $dataa['idpelanggan']; ?>"><?= $dataa['namapelanggan']; ?></option>
										<?php endforeach; ?>
									</select>
									<button name="submit" style="position:relative; left:20px;" class="btn btn-warning">Cari</button>
							   </div>
							</form>
							<form method="post" action="<?php echo base_url('admin/entrytransaksi'); ?>">
								<div class="input-group input-search">
									<label>Cari Nama</label><br/>
									<input class="form-control" style="position: relative; width: 200px;" type="text" name="nama">
									<button name="submit1" style="position:relative; left:20px;" class="btn btn-warning">Cari</button>
								</div> 
							</form>
							</div>
						</div>
					...
					<br>
					    ...
					    <table class="table table-hover" border="1">
							...
							<?php if(isset($dandi)){ ?>
							<?php foreach($dandi as $no => $idnad) : ?>
							<tr>
								<td><?= $no+1; ?></td>
								<td><?= $idnad['idpesanan']; ?></td>
								<td><?= $idnad['namapelanggan']; ?></td>
								<td><?= $idnad['namamenu']; ?></td>
								<td><?= $idnad['jumlah']; ?></td>
								<td><?= $idnad['iduser']; ?></td>
								<td><?= $idnad['status_pesanan']; ?></td>
								<td><a href="<?= base_url('admin/delete'); ?>/?id=<?= $idnad['idpesanan']; ?>" onclick="return confirm('Yakin Ingin Menghapus')">Hapus</a></td>
							</tr>
							<?php endforeach; ?>
							<a href="<?php echo base_url('admin/checkout'); ?>?id=<?php echo $idnad['idpelanggan']; ?>&idpesanan=<?php echo $idnad['idpesanan']; ?>">Checkout</a>
							<?php } ?>
						</table>
					</div>
				</div>
					...
					<br>
					    ...
					    <table class="table table-hover" border="1">
							...
							<?php if(isset($nama)){ ?>
							<?php foreach($nama as $no1 => $idnad1) : ?>
							<tr>
								<td><?= $no1+1; ?></td>
								<td><?= $idnad1['idpesanan']; ?></td>
								<td><?= $idnad1['namapelanggan']; ?></td>
								<td><?= $idnad1['namamenu']; ?></td>
								<td><?= $idnad1['jumlah']; ?></td>
								<td><?= $idnad1['iduser']; ?></td>
								<td><?= $idnad1['status_pesanan']; ?></td>
								<td><a href="<?= base_url('admin/delete'); ?>/?id=<?= $idnad1['idpesanan']; ?>" onclick="return confirm('Yakin Ingin Menghapus')">Hapus</a></td>
							</tr>
							<?php endforeach; ?>
							<a href="<?php echo base_url('admin/checkout'); ?>?id=<?php echo $idnad1['idpelanggan']; ?>&idpesanan=<?php echo $idnad1['idpesanan']; ?>">Checkout</a>
							<?php } ?>
						</table>
					</div>
				</div>
